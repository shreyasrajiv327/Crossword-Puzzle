...
var timeRemaining = 180; // Set the countdown time in seconds (e.g., 10 minutes)
...
function resetTimer() {
    timeRemaining = 300; // Reset to 10 minutes
    document.getElementById('timer').innerText = formatTime(timeRemaining);
}
    ...
    if (valToCheck === currVal) {
        $('.position-' + activePosition + ' input')
            .addClass('done') // Add 'done' class to inputs with correct answers
            .removeClass('active'); // Remove 'active' class if it was applied
        ...
        score++;
		var percentage = (solved.length / 8) * 100;
    ...
    //             alert("Time is up!");
    //             // Add any additional logic you need when the time is up
    //         }
    //     }, 1000);
    // }
    // $(document).ready(function () {
    //     var threeMinutes = 60 * 3,
    //         display = $('#timer'); // Ensure there's an element with id="timer" in your HTML
